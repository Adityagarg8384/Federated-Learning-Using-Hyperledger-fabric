...
export const getAffiliation = async (org) => {
    return org == "Org1" ? 'org1.department1' : 'org2.department1'
}

export const getRegisteredUser = async (username, userOrg, isJson) => {
...
export const isUserRegistered = async (username, userOrg) => {
    const walletPath = await getWalletPath(userOrg)
    const wallet = await Wallets.newFileSystemWallet(walletPath);
    console.log(`Wallet path: ${walletPath}`);
...
export const enrollAdmin = async (org, ccp) => {
...
export const registerAndGerSecret = async (username, userOrg) => {
    let ccp = await getCCP(userOrg)
...
export const python= async(userOrg)=>{
    let ccp= await getCCP(userOrg)
    const caURL = await getCaUrl(userOrg, ccp)
    const ca = new FabricCAServices(caURL);
...
// export {getCCP,getAffiliation,getCaInfo,getCaUrl,getRegisteredUser,getWalletPath,isUserRegistered}
// exports.getRegisteredUser = getRegisteredUser
// exports.getCCP=getCCP
// exports.getWalletPath=walletPath
